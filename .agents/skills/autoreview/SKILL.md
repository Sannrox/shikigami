---
name: autoreview
description: "Pre-commit/ship code review: Codex default; optional Claude, Pi, Droid, Copilot, Kiro or OpenCode."
---
...
```bash
"$AUTOREVIEW" --reviewers codex,claude,pi,droid,kiro
```
...
|--------|---------------|-------------|
| **codex** (default) | `gpt-5.6-sol` | OpenAI's explicit GPT-5.6 Sol model ID |
...
|--------|------------|-------------------|---------------|-----------------|
| **codex** (default) | `codex --model X exec ...` | `gpt-5.6`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5` | `-c model_reasoning_effort=Y` | `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max` |
...
# Current GPT-5.6 variants for different cost/quality targets
"$AUTOREVIEW" --engine codex --model gpt-5.6-sol --thinking max
"$AUTOREVIEW" --engine codex --model gpt-5.6-terra --thinking high
"$AUTOREVIEW" --engine codex --model gpt-5.6-luna --thinking high
...
| `AUTOREVIEW_<ENGINE>_MODEL` | Per-engine model override, for example `AUTOREVIEW_CODEX_MODEL=gpt-5.6-terra` |
